/****** Object:  StoredProcedure [NBA].[SP_InsertInto_PlayerGamePositionStats_PerPlayerSeason_Percent]    Script Date: 7/1/2022 2:52:11 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE procedure [NBA].[SP_InsertInto_PlayerGamePositionStats_PerPlayerSeason_Percent] as

drop table  NBA.TBL_PlayerGamePositionStats_PerPlayerSeason_Percent

;
with t0 as
(
	select *, [Group] = '>= 2500' from [NBA].[VW_PlayerGamePositionStats_Average] where [Time Played (Minutes)] >= 2500
	union all select *, [Group] = '>= 2000' from [NBA].[VW_PlayerGamePositionStats_Average] where [Time Played (Minutes)] >= 2000
	union all select *, [Group] = '>= 1500' from [NBA].[VW_PlayerGamePositionStats_Average] where [Time Played (Minutes)] >= 1500
	union all select *, [Group] = '>= 1000' from [NBA].[VW_PlayerGamePositionStats_Average] where [Time Played (Minutes)] >= 1000
	union all select *, [Group] = '>= 0' from [NBA].[VW_PlayerGamePositionStats_Average] where [Time Played (Minutes)] >= 0
)
select r.* 
...
into NBA.TBL_PlayerGamePositionStats_PerPlayerSeason_Percent
from t0 r
left join [NBA].[VW_PlayerGamePositionStats_MaxAndMin] mm
on r.[Group] = mm.[group]
and r.Position = mm.Position
